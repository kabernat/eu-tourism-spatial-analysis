...
body {
  background-image: url('gdansk.jpg'); 
...
.success-box { background-color: #d4edda; border-left: 5px solid #28a745; padding: 12px 16px; margin: 12px 0; border-radius: 4px; font-size: 0.95em; }
.info-box { background-color: #e8f4fd; border-left: 5px solid #2196F3; padding: 12px 16px; margin: 12px 0; border-radius: 4px; font-size: 0.95em; }
.result-box { background-color: #fff3cd; border-left: 5px solid #ffc107; padding: 12px 16px; margin: 12px 0; border-radius: 4px; font-size: 0.95em; }
.warning-box { background-color: #f8d7da; border-left: 5px solid #dc3545; padding: 12px 16px; margin: 12px 0; border-radius: 4px; font-size: 0.95em; }
.metric-grid { display: flex; gap: 12px; flex-wrap: wrap; margin: 12px 0; }
.metric-card { background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 12px 18px; text-align: center; min-width: 130px; flex: 1; }
...
Parametr λ = `r round(model_SAR_RE$arcoef["lambda"], 4)` oznacza, że intensywność turystyki w danym regionie jest silnie powiązana ze średnią intensywnością w regionach sąsiednich — autoregresja przestrzenna jest wysoce istotna. Parametr φ = `r round(model_SAR_RE$errcomp["phi"], 2)` wskazuje, że zmienność między regionami jest ok. `r round(model_SAR_RE$errcomp["phi"])` razy większa niż zmienność wewnątrz regionu w czasie, co potwierdza zasadność efektów losowych.
...
Parametr λ = `r round(model_SDM_RE$arcoef["lambda"], 4)` potwierdza silną autoregresję przestrzenną — intensywność turystyki w regionie zależy od intensywności u sąsiadów. Wartość jest zbliżona do modelu SAR-RE, co wskazuje na stabilność tego parametru niezależnie od specyfikacji. Parametr φ = `r round(model_SDM_RE$errcomp["phi"], 2)` oznacza, że zmienność między regionami jest ok. `r round(model_SDM_RE$errcomp["phi"])` razy większa niż zmienność wewnątrz regionu w czasie.
...
<div class="warning-box">
⚠️ <strong>Błędy standardowe NaN:</strong> Współczynniki β (tabela powyżej) są oszacowane poprawnie, ale dla parametrów ρ i φ algorytm nie był w stanie obliczyć błędów standardowych. Dzieje się tak dlatego, że wartość ρ = `r round(model_SEM_RE$errcomp["rho"], 3)` jest bardzo bliska górnej granicy dopuszczalnego zakresu (1,0), co powoduje niestabilność numeryczną przy obliczaniu macierzy kowariancji. W praktyce oznacza to, że model SEM-RE nie nadaje się do wnioskowania statystycznego o parametrach przestrzennych i nie może być traktowany jako model finalny.
</div>
...
**H₀:** θ = 0 (lagi WX wspólnie nieistotne, SAR-RE wystarczy)
**H_A:** θ ≠ 0 (SDM-RE jest właściwą specyfikacją)

$$LR = 2 \cdot (logL_{SDM-RE} - logL_{SAR-RE}) \sim \chi^2_5$$

```{r lr-sdm-sar, results='hide'}
LR_sdm   <- 2 * (model_SDM_RE$logLik - model_SAR_RE$logLik)
pval_sdm <- pchisq(LR_sdm, df = 5, lower.tail = FALSE)
```

```{r lr-sdm-sar-table, echo=FALSE}
data.frame(
...
```{r wald-table, echo=FALSE}
data.frame(
  Hipoteza   = c("λ = 0 (brak autoregresji przestrzennej)",
                 "θ = 0 (lagi WX wspólnie nieistotne)"),
  Statystyka = c(round(wald_W_lam, 4),
                 round(wald_W,     4)),
  df         = c(1, 5),
  `p-value`  = c(formatC(wald_p_lam, format = "e", digits = 2),
                 formatC(wald_p,     format = "e", digits = 2)),
  Wniosek    = c(
    ifelse(wald_p_lam < 0.05, "Odrzucamy H₀ ✅", "Brak podstaw do odrzucenia H₀"),
    ifelse(wald_p     < 0.05, "Odrzucamy H₀ ✅", "Brak podstaw do odrzucenia H₀")
  ),
  check.names = FALSE
) |>
  kbl(caption = "Testy Walda ex-post — model SDM-RE",
      align = c("l","r","r","r","l")) |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"), full_width = FALSE) |>
  row_spec(1:2, background = "#d4edda") |>
  pack_rows("Autoregresja przestrzenna", 1, 1) |>
  pack_rows("Lagi przestrzenne WX (spójność z LR, sekcja 10.2)", 2, 2)
```

<div class="info-box">
ℹ️ <strong>Interpretacja wyników:</strong><br><br>
Wald dla λ = 0: W = `r round(wald_W_lam, 2)`, p < 2,2×10⁻¹⁶ — autoregresja przestrzenna jest silnie istotna, co potwierdza zasadność modelu SDM-RE względem pooled OLS.<br><br>
Wald dla θ = 0: W = `r round(wald_W, 4)`, p = `r formatC(wald_p, format="e", digits=2)` — lagi WX są wspólnie istotne. Wynik jest spójny z testem LR (sekcja 10.2, p = `r round(pval_sdm, 3)`): dwie niezależne metody prowadzą do tego samego wniosku, co wzmacnia wiarygodność wyboru SDM-RE względem SAR-RE.<br><br>
Parametr φ = σ²_α / σ²_ε = `r round(model_SDM_RE$errcomp["phi"], 2)` — zmienność między regionami jest ok. `r round(model_SDM_RE$errcomp["phi"])` razy większa niż zmienność wewnątrz regionu w czasie, co potwierdza zasadność efektów losowych.
</div>

## 10.4 Ocena modelu SEM-RE i próba estymacji SARAR-RE

<div class="warning-box">
⚠️ <strong>Model SEM-RE — nie nadaje się jako model finalny</strong><br><br>
...
<div class="warning-box">
⚠️ <strong>Model SARAR-RE — nie udało się go wyestymować</strong><br><br>
Ponieważ reszty modelu SDM-RE wykazują autokorelację przestrzenną (sekcja 10.6), podjęto próbę estymacji modelu SARAR-RE, który łączy autoregresję przestrzenną z autokorelacją błędów. Model ten nie dał się jednak wyestymować — funkcja <code>spml()</code> z pakietu <code>splm</code> zwróciła błąd numeryczny. Jest to znane ograniczenie tego pakietu przy dużej liczbie regionów (N = 220) i silnych zależnościach przestrzennych, a nie błąd w specyfikacji modelu. SARAR-RE pozostaje metodologicznie uzasadnioną alternatywą, ale niedostępną w tym środowisku obliczeniowym.
</div>
...
## 10.6 Test Morana I na resztach modelu SDM-RE
...
<ol>
<li>Testy ex-ante: Robust LM-SAR istotnie bardziej znaczący niż Robust LM-SEM (sekcja 8.4), co wskazuje na autoregresję przestrzenną jako właściwy mechanizm.</li>
<li>Testy BSK (sekcja 8.6): potwierdzają konieczność uwzględnienia zarówno autoregresji przestrzennej, jak i efektów losowych.</li>
<li>Test LR (sekcja 10.1): SDM-RE vs pooled OLS — LR = `r round(LR_sdm_re, 2)`, p < 2,2×10⁻¹⁶ — model przestrzenny istotnie lepszy od OLS.</li>
<li>Test LR (sekcja 10.2): SDM-RE vs SAR-RE — LR = `r round(LR_sdm, 3)`, p = `r round(pval_sdm, 3)` — lagi WX wspólnie istotne, SDM-RE lepszy od SAR-RE.</li>
<li>Test Walda (sekcja 10.3): λ = 0 — W = `r round(wald_W_lam, 2)`, p < 2,2×10⁻¹⁶; θ = 0 — W = `r round(wald_W, 4)`, p = `r formatC(wald_p, format="e", digits=2)` — oba parametry przestrzenne istotne.</li>
<li>Porównanie miar dopasowania (sekcja 10.5): SDM-RE ma najwyższy logLik i najniższy AIC spośród wszystkich estymowanych modeli; SEM-RE ma problem z wnioskowaniem o parametrach przestrzennych.</li>
</div>
...
**PKB per capita (`ln_gdp_pc`):** Wzrost PKB per capita o 1% zwiększa intensywność turystyki o ok. `r round(e_gdp$Bezposredni, 2)`% w danym regionie (efekt bezpośredni) i o ok. `r round(e_gdp$Posredni, 2)`% w regionach sąsiednich (efekt pośredni). Łączny efekt całkowity wynosi `r round(e_gdp$Calkowity, 2)`%. Wszystkie efekty istotne na poziomie p < 0,001.

**Gęstość zaludnienia (`ln_pop_density`):** Wzrost gęstości zaludnienia o 1% zmniejsza intensywność turystyki o ok. `r round(abs(e_pop$Bezposredni), 2)`% w danym regionie i o ok. `r round(abs(e_pop$Posredni), 2)`% w regionach sąsiednich. Efekt całkowity: `r round(e_pop$Calkowity, 2)`%. Wszystkie efekty istotne na poziomie p < 0,001.
...
**Powierzchnia regionu (`ln_area_km2`):** Wzrost powierzchni regionu o 1% zmniejsza intensywność turystyki o ok. `r round(abs(e_area$Bezposredni), 2)`% w danym regionie i o ok. `r round(abs(e_area$Posredni), 2)`% w regionach sąsiednich. Efekt całkowity: `r round(e_area$Calkowity, 2)`%. Większe regiony mają niższą intensywność, bo infrastruktura turystyczna jest bardziej rozproszona. Wszystkie efekty istotne na poziomie p < 0,001.
