...
cor_df[] <- lapply(cor_df, function(x) cell_spec(x, background = "white", color = "black"))
cor_df[1, 3] <- cell_spec(cor_matrix[1, 3], bold = TRUE, color = "white", background = "#dc3545")
cor_df[3, 1] <- cell_spec(cor_matrix[3, 1], bold = TRUE, color = "white", background = "#dc3545")

cor_df |>
  kbl(escape = FALSE, caption = "Macierz korelacji zmiennych objaśniających (2019)") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"), full_width = FALSE) |>
...
beta_coef <- model_SDM_RE$coefficients

effects_full <- effects_df |>
  left_join(mc_results, by = "Zmienna") |>
  mutate(
    beta_val    = as.numeric(beta_coef[vars]),
...
data.frame(
  Zmienna             = vars,
  `β (SDM-RE)`        = round(beta_coef[vars], 5),
